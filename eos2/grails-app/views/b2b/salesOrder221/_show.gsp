...
<table>
	...
	<tbody>
		...
		<tr class="odd">
			...
			<td>
				<span id="department">
					${include(controller : 'salesOrder221Ajax', action : 'getDealerDepartment', params : [dealerSerialNumber : salesOrderInstance?.dealer?.serialNumber] )}
				</span>
			</td>
	...
	<table id="salesOrder_detail_list">
        ...
        <tbody>
			<g:each in="${salesOrderInstance.salesOrderDetails}" status="i" var="salesOrderDetailInstance">
				<g:render template="/b2b/salesOrder221/showDetail" model="['salesOrderDetailInstance':salesOrderDetailInstance,'i':i]"/>
			</g:each>
		</tbody>
		<tfoot>
			<td>&nbsp;</td>
...
<!--workflow history-->
<div class="dialog">
	<g:include controller="salesOrder221Ajax" action="workflowHistory" params="[salesOrderId: salesOrderInstance?.id, ownerId : salesOrderInstance?.owner?.id]" />
</div>
<!--outbound history-->
<div class="dialog">
	<g:include controller="salesOrder221Ajax" action="outBoundMessage" params="[salesOrderId: salesOrderInstance?.id]" />
</div>
