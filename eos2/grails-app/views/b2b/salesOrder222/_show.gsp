...
<table>
	...
	<tbody>
		...
		<tr class="odd">
			...
			<td>
				<span id="department">
					${include(controller : 'salesOrder222Ajax', action : 'getDealerDepartment', params : [dealerSerialNumber : salesOrderInstance?.dealer?.serialNumber] )}
				</span>
			</td>
	...
	<table id="salesOrder_detail_list">
    	...
    	<thead>
        	<tr>
				...
				<th class="number"><label><g:message code="salesOrderDetail.price.label" default="Price" /></label></th>
                <th>
				...
				<th class="number"><label><g:message code="salesOrderDetail.amount.label" default="Amount" /></label></th>
            </tr>
        </thead>
        <tbody>
			<g:each in="${salesOrderInstance.salesOrderDetails}" status="i" var="salesOrderDetailInstance">
				<g:render template="/b2b/salesOrder222/showDetail" model="['salesOrderDetailInstance':salesOrderDetailInstance,'i':i]"/>
			</g:each>
		</tbody>
		<tfoot>
			<td>&nbsp;</td>
...
<!--workflow history-->
<div class="dialog">
	<g:include controller="salesOrder222Ajax" action="workflowHistory" params="[salesOrderId: salesOrderInstance?.id, ownerId : salesOrderInstance?.owner?.id]" />
</div>
<!--outbound history-->
<div class="dialog">
	<g:include controller="salesOrder222Ajax" action="outBoundMessage" params="[salesOrderId: salesOrderInstance?.id]" />
</div>
