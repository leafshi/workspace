			...
			<li>
				...
				<g:hiddenField name="salesOrderDetails[${i}].deliveryCycle" value="${salesOrderDetailInstance?.deliveryCycle}" />
       			...
       			<span name="salesOrderDetails[${i}].firstDeliveryDate" style="display:none;">
       				${include(controller : 'salesOrder220Ajax', action : 'firstDeliveryDate', params : [productId : salesOrderDetailInstance?.product?.id] )}
       			</span>
       		</li>
		</ul>
	</td>
...
<tr class="${(i % 2) == 0 ? 'odd' : 'even'} hide_detail" parent="${salesOrderDetailInstance?.serialNumber}" coordinate="${i},${j}">
    ...
    <td name="product">
    	<ul>
			...
			<li>
				<g:message code="category.warehouse.label"/> : 
				${include(controller : 'salesOrder220Ajax', action : 'productCategoryOfCG', params : [productId : salesOrderDetailDetailInstance?.product?.id] )}
			</li>
		</ul>
