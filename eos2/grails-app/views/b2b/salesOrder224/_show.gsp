...
<table>
	...
	<tbody>
		...
		<tr class="odd">
			...
			<td>
				<span id="department">
					${include(controller : 'salesOrder224Ajax', action : 'getDealerDepartment', params : [dealerSerialNumber : salesOrderInstance?.dealer?.serialNumber] )}
				</span>
			</td>
	...
	<table id="salesOrder_detail_list">
        ...
        <tbody>
			<g:each in="${salesOrderInstance.salesOrderDetails}" status="i" var="salesOrderDetailInstance">
				<g:render template="/b2b/salesOrder224/showDetail" model="['salesOrderDetailInstance':salesOrderDetailInstance,'i':i]"/>
			</g:each>
		</tbody>
		<tfoot>
			<td>&nbsp;</td>
...
<!--workflow history-->
<div class="dialog">
	<g:include controller="salesOrder224Ajax" action="workflowHistory" params="[salesOrderId: salesOrderInstance?.id, ownerId : salesOrderInstance?.owner?.id]" />
</div>
<!--outbound history-->
<div class="dialog">
	<g:include controller="salesOrder224Ajax" action="outBoundMessage" params="[salesOrderId: salesOrderInstance?.id]" />
</div>
