		...
		Workflow.withTransaction{ status ->
			try {
				for(recordType in recordTypes){
					if(Workflow.countByDepartmentAndRecordType(branch, recordType) == 0){
						//创建工作流
						...
						//创建工作流步骤
						def step_10 = new WorkflowStep(workflow:workflowInstance, serialNumber : '10', name : '草稿', lockRecord:false, isBegin:true, isEnd :false).save(flush:true)
						def step_11 = new WorkflowStep(workflow:workflowInstance, serialNumber : '11', name : '未通过', lockRecord:false, isBegin:false, isEnd :false).save(flush:true)
			
						def step_20 = new WorkflowStep(workflow:workflowInstance, serialNumber : '20', name : '待审批（办事处）', assignee : branchGroup, lockRecord:true, isBegin:false, isEnd :false).save(flush:true)

						def step_30 = new WorkflowStep(workflow:workflowInstance, serialNumber : '30', name : '待审批（商务部）', assignee : commercialGroup, lockRecord:true, isBegin:false, isEnd :false).save(flush:true)
						def step_90 = new WorkflowStep(workflow:workflowInstance, serialNumber : '90', name : '终止', lockRecord:true, isBegin:false, isEnd :true).save(flush:true)
						def step_91 = new WorkflowStep(workflow:workflowInstance, serialNumber : '91', name : '审批通过', lockRecord:true, isBegin:false, isEnd :true).save(flush:true)
						def step_92 = new WorkflowStep(workflow:workflowInstance, serialNumber : '92', name : '审批未通过', lockRecord:true, isBegin:false, isEnd :true).save(flush:true)
	...
	*/
    def initWorkflow2(Department commercial) {
		...
		Workflow.withTransaction{ status ->
			try {
				for(recordType in recordTypes){
					if(Workflow.countByDepartmentAndRecordType(commercial, recordType) == 0){
						//创建工作流
