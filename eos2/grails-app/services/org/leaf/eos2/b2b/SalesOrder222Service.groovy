...
class SalesOrder222Service {
    ...
    def recordType() {
        def recordType222 = RecordType.withCriteria(uniqueResult:true){
            projections{
                property('id')
            }
	...
	def insert(SalesOrder salesOrderInstance){
		...
		while (error_times < error_times_limit){
			salesOrderInstance.serialNumber = "EB222-" + new Date().format('yyyyMMdd') + '-' + this.lastSerialNumber()
    ...
    def update(SalesOrder salesOrderInstance, Object params) {
		...
		SalesOrder.withTransaction{ status ->
			try{
				...
				success = false
				log.error("update salesorder 222 for instance ${salesOrderInstance} error : ${e}")
